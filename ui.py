...
class CreateClienteWindow(Toplevel,CenterWidgetMixin):
    ...
    def validate(self,event,index):
        ...
        valido = helpers.dni_validate(valor, db.Clientes.lista) if index == 0 else valor.isalpha() and len(valor)>=2 and len(valor)<=30
        event.widget.configure({"bg":"Green" if valido else "Red"})
        #Boton
        self.validaciones[index] = valido
        self.crear.config(state=NORMAL if self.validaciones==[1,1,1] else DISABLED)

class EditClienteWindow(Toplevel,CenterWidgetMixin):
    def __init__(self,parent):
        super().__init__(parent)
